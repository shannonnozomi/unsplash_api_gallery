...
function createFavImage(newImage) {
  ...
  favoriteIcon.innerHTML =
    "<span class='heartActive heartFull iconify' data-icon='dashicons:heart' data-inline='false'></span>";
  favoriteIcon.onclick = function () {
    favorited(imageDiv, newImage);
  };
...
function favorited(imageDiv, newImage) {
  imageDiv.animate([{ opacity: "-1" }], { duration: 250 });
...
function clearFunction() {
  if (favImagesArray === undefined || favImagesArray.length == 0) {
    ...
    divForImg.animate([{ opacity: "-1" }], { duration: 250 });
    function timeFunction() {
      setTimeout(function () {
        divForImg.remove();
      }, 200);
